...
class CandleSticks(object):
    ...
    def request_slice(self, start, end, granularity):
        ...
        for retry_count in range(0, retries):
            ...
            response = requests.get(self.uri, {
                'start': CandleSticks.__date_to_iso8601(start),
                'end': CandleSticks.__date_to_iso8601(end),
    ...
    def price_series(self, start, end, granularity):
        df = self.fetch(start,end,granularity)
        return pandas.Series(df['close'].values)
